...
for(const collection of curation.collections){
 ...
 const cards=works.map(art=>'<article class="art-card'+(art.id==='drmmetrappen-til-manen'?' art-card-landscape-feature':'')+'"><a href="/verk/'+encodeURIComponent(art.id)+'/" aria-label="Explore '+encode(art.title)+'"><span class="art-photo"><img loading="lazy" src="'+encode(art.thumb)+'" alt="'+encode(art.title)+'" width="'+art.width+'" height="'+art.height+'"></span><span class="art-card-meta"><span><span class="art-title">'+encode(art.title)+'</span><span class="art-category">'+encode(art.category)+' · '+(art.digital_available===false?'Gallery preview · Not for sale':'Digital edition €50')+'</span></span><span class="art-number">'+String(art.number).padStart(3,'0')+'</span></span></a></article>').join('\n');
