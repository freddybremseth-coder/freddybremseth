(()=>{'use strict';
 ...
 const state={art:[],styles:[],collections:[],filtered:[],count:18,category:'all',style:'all',orientation:'all',price:'all',search:'',sort:'collection',selected:null,lang:'en',checkout:false,print:{byArtworkId:{},defaultPrintUrl:''}};
 ...
 function render(){
  ...
  $('active-style-description').textContent=collection?collection.description:'Four signature collections plus our studio archive. Further refine with style, orientation, price or a title search.';
  $('load-more').hidden=true;
  if(grouped){
   const narrowed=!!(state.search||state.style!=='all'||state.orientation!=='all'||state.price!=='all');
 ...
 function updateBuy(){const btn=$('checkout-button');btn.disabled=!state.checkout||!$('digital-consent').checked;btn.textContent=state.checkout?text('buy'):text('unavailable')}
 function openArt(art,push){state.selected=art;$('dialog-image').src=art.image;$('dialog-image').alt=art.title;$('dialog-title').textContent=art.title;$('dialog-index').textContent='ARTWORK / '+String(art.number).padStart(3,'0');$('dialog-category').textContent=art.category;$('dialog-collection').textContent=collectionName(art)+' · Digital artwork €'+(art.price_cents/100).toFixed(0);$('dialog-story').textContent=art.story;$('dialog-edition').textContent=art.edition;$('dialog-size').textContent=art.width+' × '+art.height+' px (public preview source)';$('digital-consent').checked=false;updateBuy();const url=printLinkFor(art);$('print-link').hidden=!url;$('print-placeholder').hidden=!!url;if(url)$('print-link').href=url;
 ...
 $('clear-filters').addEventListener('click',()=>{state.category='all';state.style='all';state.orientation='all';state.price='all';state.search='';state.sort='collection';state.count=18;
  $('art-search').value='';$('art-sort').value='collection';$('style-filter').value='all';$('orientation-filter').value='all';$('price-filter').value='all';render()});
