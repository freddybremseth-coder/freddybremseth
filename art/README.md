...
The admin accepts a JPEG, PNG, WebP or ZIP of these files (at most 50 MB per image, 500 MB per ZIP, 100 artworks per batch). For an existing artwork, choose **Attach or replace an existing artwork's source files**, then select the exact artwork in the review list. Uploading creates a reduced public WebP preview in `art-previews` and registers the full uploaded image privately in `art-originals` with `art_gallery_masters` metadata. A registration does not prove the private file has been audited or is fit for large prints. The catalogue now displays the private-master registration state and lets administrators edit each work's description without re-uploading an image.
