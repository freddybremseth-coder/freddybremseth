...
if(art.length<50||masters.length!==art.length)throw Error('Gallery or private master manifest incomplete');
...
for(const m of masters){if(!ids.has(m.id)||!/^[a-z0-9-]+\.(jpg|png)$/.test(m.storage_path))throw Error('Invalid protected master '+m.id)}
...
console.log('PASS: '+art.length+' works sorted into '+styles.length+' curated artistic styles, '+publicFiles.length+' public previews, canonical artwork pages, protected source mapping, €50 server-authoritative price');
